...
const funcs = {
  postEvaluation: async () => {
    let response = await fetch(`${baseURL}/api/Evaluation`, {
      method: 'POST',
      body: JSON.stringify({
        styleOfEvaluation: 'WriteCode',
        link: 'www.test.com'
      }),
      headers: {
  ...
  getEvaluation: async () => {
    let response = await fetch(`${baseURL}/api/Evaluation?id=${reply._id}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      },
