...
const funcs = {
  postMemberReviewsPartner: async () => {
    let response = await fetch(
      `${baseURL}/api/Member/reviewPartner`,
      {
        method: "POST",
        body: JSON.stringify({task : "5ca002ae15f5040438cf1fa9",
  ...
  deleteMemberReviewsPartner: async () => {
    let response = await fetch(
      `${baseURL}/api/review?id=${reply._id}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json"
  ...
  const postTest=test("post member reviews partner", async () => {
    ...
    expect(response.reviewer).toEqual("5c7581a12357f33970c4d757") && expect(response.reviewee).toEqual("5c9494d3f0c6c02014be6b5f")
